	...
	secondCheck = compileFinal "
		while{_this select 0}do{
			...
			if(isESVP && (vehicle player != player))then{
				if(isNil'plrProtAdded')then{
					if(noFiredCheckPlr)then{_nFcP=true}else{_nFcP=false};
					_nFcP call addProtPlr
				};
				if(noFiredCheckVeh)then{_nFcV=true}else{_nFcV=false};
				if(!_nFcV)then{
